...
export default function MargemPage() {
  const [data, setData] = useState<MargemProduto[]>([]);
  ...
  const fetchData = () => {
    ...
    api
      .get(`/analytics/margem-por-produto?${params.toString()}`)
      .then((res) => {
        setData(res.data.itens || []);
        setError('');
      })
      .catch(() => setError('Erro ao carregar margem por produto'))
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Vendas (qtd)</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-right">{item.vendas_qtd}</td>
